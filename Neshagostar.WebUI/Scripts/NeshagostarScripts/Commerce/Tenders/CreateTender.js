$(function () {
    ...
    $("#btnSubmit").on("click", function () {  ///  binds 2 condition to this button
        if (orderItems.length == 0) {   /// if orderItems list is empty it means no product has been selected so the button will not submit
            alert("هیچ آیتمی برای مناقصه وارد نشده است");
...
function createBindingObject() {
    ...
    var carriageCost = $('#carriageCost').val();
    var guarantyCost = $('#guarantyCost').val();
    var inspectioncost = $("#inspectioncost").val();
    var predictionPrice = $("#predictionPrice").val();
    var participatingGuarantyPrice = $("#participatingGuarantyPrice").val();
    var tenderTitle = $("#tenderTitle").val();
...
function getProductsChosenBefore(tenderId) {
    return $.ajax({
        type: "GET",
        url: "/commerce/Tenders/getProductsChosen",
